...
class FinancialAnalyzerMLflowModel(mlflow.pyfunc.PythonModel):
    def load_context(self, context):
        ...
        perplexity_api_key = f"{api_key['key']['perplexity']}"
        alpha_advantage_key = f"{api_key['key']['alphaadvantage']}"
...
def get_api_keys():
    perplexity_key =f"{api_key['key']['perplexity']}"
    alpha_key = f"{api_key['key']['alphaadvantage']}"
...
if __name__ == "__main__":
    ...
    if action == "test_analyzer":
        ...
        if not PERPLEXITY_API_KEY or not ALPHA_ADVANTAGE_KEY:
            ...
            code_paths = [current_script_name, "strategy.py", "secret.yml"]
            print('start run')
